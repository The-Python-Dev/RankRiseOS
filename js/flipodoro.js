/* Flipodoro core port — timer.py (v3 Timestamp Anti-Throttle Engine) */
...
class PomodoroTimer {
  constructor(opts = {}) {
    ...
    // Absolute Wall-Clock Target Timestamp (Anti-Throttle Anchor)
    ...
    if (typeof document !== 'undefined') {
      document.addEventListener('visibilitychange', () => {
        if (document.visibilityState === 'visible' && this._state === TimerState.RUNNING) {
          this.tick();
        }
      });
    }
  ...
  start() {
    if (this._state === TimerState.IDLE || this._state === TimerState.PAUSED) {
      // Calculate absolute target end timestamp in physical time
      this._targetEndTime = Date.now() + (this._remainingSeconds * 1000);
  ...
  _computeRemaining() {
    if (this._targetEndTime == null) return this._remainingSeconds;
    const remainingMs = this._targetEndTime - Date.now();
